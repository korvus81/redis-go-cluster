...
type Options struct {
	...
	Password   string   // Cluster password, or ""
...
func NewCluster(options *Options) (*Cluster, error) {
	...
	for i := range options.StartNodes {
		...
		if err != nil {
			fmt.Printf("Error in NewCluster for node %+v, err =%v\n", node, err)
			continue
		} else {
...
func (cluster *Cluster) update(node *redisNode) error {
	ret, err1 := node.do("CLUSTER", "SLOTS")
	info, err := Values(ret, err1)
	if err != nil {
		fmt.Printf("Error for node %+v, err =[%v], info=[%v], ret=[%v]\n", node, err, info, ret)
		return err
	}
	...
	for _, i := range info {
		m, err := Values(i, err)
		if err != nil || len(m) < 3 {
			fmt.Printf("len(m) < 3 or err, len=%v, err=%v", len(m), err)
			return errFormat
		}
		...
		if err != nil || (len(t) != 2 && len(t) != 3) {
			fmt.Printf("m[2] != Values, m[2]=%v, len(t)=%v", m[2], len(t))
			return errFormat
		}
